...
namespace commands
{
        ...
        _cmd{app.add_subcommand("read", "Read data from flash")}
    {
        ...
        _cmd->add_option("-s,--start", _start, "Read range: start offset")
            ->required()
            ->transform(CLI::AsSizeValue(false));

        _cmd->add_option("-e,--end", _end, "Read range: end offset")
            ->required()
            ->transform(CLI::AsSizeValue(false));

        _cmd->add_option("-f,--file", _outputFilePath, "Output file path")
            ->required();
    }
